/* FOUR_SITE_ADDITIONS_START: refreshed Mangabz, Manhuagui, Zero */
comicsWebInfo.unshift({
  domain:["mangabz.com","www.mangabz.com"],homepage:"https://www.mangabz.com/",webName:"Mangabz",
  comicNameCss:"p.detail-info-title",chapterCss:"#chapterlistload",readtype:1,useFrame:true,
  headers:{referer:"https://www.mangabz.com/"},searchTemplate_1:{search_add_url:"search?title=",alllist_dom_css:".container .mh-list",minlist_dom_css:"li",img_src:"src"},
  downHeaders:{referer:"https://www.mangabz.com/"},
  getImgs:async function(html,task){
    ...
    try{
      ...
      if(!total||total>1000||!doc.querySelector('a[href="javascript:ShowNext();"]'))throw new Error("Mangabz page list is unavailable");
      const images=[];
      for(let page=1;page<=total;page++){
        ...
        for(let attempt=0;attempt<80;attempt++){
          ...
          if(current===page&&/^https?:\/\//i.test(src)&&!/\/loading\./i.test(src)&&src!==images[images.length-1]){
            found=new URL(src,task.url).href;break;
          }
...
comicsWebInfo.push({
  domain:["manhuagui.com","www.manhuagui.com"],homepage:"https://www.manhuagui.com/",webName:"\u770b\u6f2b\u753b",
  comicNameCss:"h1",chapterCss:".chapter-list",chapterNameReg:/title="([^"]+)"/,readtype:1,useFrame:false,
  readerTabMode:"manhuagui",downHeaders:{referer:"https://www.manhuagui.com/"},
  getImgs:async function(html,task){
    ...
    const record={id,status:"waiting",readerUrl,images:[],error:"",cancelled:false,updatedAt:Date.now()};
    let tab=null;
    ...
    try{
      ...
      tab=GM_openInTab(readerUrl+"#tencomic-mg-task="+id,{active:false,insert:true,setParent:true});
      for(let attempt=0;attempt<900;attempt++){
        if(task?.signal?.aborted){const current=GM_getValue(key,record);GM_setValue(key,{...current,cancelled:true,status:"cancelled"});throw new Error("Download cancelled")}
    ...
    }finally{GM_deleteValue(key);try{tab?.close?.()}catch(error){}}
  }
},{
  domain:["zerobyw33.com","www.zerobyw33.com"],homepage:"https://www.zerobyw33.com/pc/pc/",webName:"Zero\u642c\u8fd0\u7f51",
  comicNameCss:"main h1",chapterCss:"main .grid.grid-cols-2",readtype:1,useFrame:true,
  getComicInfo:async function(){
    ...
    for(const link of links){
      ...
      if(!chapters.has(url))chapters.set(url,{comicName:title,chapterName:name,chapterNumStr:"",url,readtype:this.readtype,isPay:false,isSelect:false});
    }
    return [...chapters.values()];
  },
  downHeaders:{referer:"https://www.zerobyw33.com/"},
  getImgs:async function(html,task){
    const frame=document.getElementById(task.frameId);
    ...
    try{
      ...
      for(let attempt=0;attempt<40;attempt++){
        if(task.signal?.aborted)throw new Error("Download cancelled");
        const images=[...doc.querySelectorAll("#image-container img.manga-image")]
          .map(img=>img.getAttribute("src")||"")
          .filter(src=>src&&!/loading|placeholder/i.test(src))
          .map(src=>new URL(src,task.url).href)
